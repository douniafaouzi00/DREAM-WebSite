import {
    Button,
    Container, Icon,
    Input, Modal, ModalBody, ModalFooter, ModalHeader, notify, TextArea
...
function ButtonMyTopics() {
    let navigate = useNavigate()
    function toMyTopics(){
        navigate("/farmer/forum/mytopics")
    }
    return(
        <button className='btn btn-secondary' type='button' id='button-3' onClick={()=>toMyTopics()} style={{marginLeft:10}}>
            My topics
        </button>
    )
}

export class FarmerForum extends React.Component{
    constructor(props) {
        super(props);
    ...
    getTopicByWord = () => {
        this.client.GetAllTopicByWord(this.state.search)
            .then((response) => {
                if(response.data.code==200){
                    this.setState({
                        topics: response.data.results[0],
    ...
    topics=()=>{
        if(this.state.topics){
            return this.state.topics.map((data) => <Topic key={data.topicId} farmer={this.props.farmer} data={data} getTopic={()=>this.getTopic()}/>)
        }
    }
        ...
        return(
            <div>
                <Container style={{paddingTop:20,paddingBottom:20, minHeight:500}} >
                    <h4 style={{fontSize:35}}>Topics</h4>
